...
def convert_imu_orientation_to_os(subject, f_type, orientation_mt, fs, stat_flag):
	...
	time_id = np.arange(0, orientation_mt['shank_r'].shape[0], 1)/fs
	...
	for segment in orientation_mt.keys():
		...
		output_key = constant_mc10.MC10_TO_OPENSENSE_MAP[segment]
		output[output_key] = ["{},{},{},{}".format(*row) for row in quat]

	if stat_flag:
		orientation_fn = f's{subject}_cal_{f_type}_orientation.sto'
	else:
		orientation_fn = f's{subject}_{f_type}_orientation.sto'
...
def os_calibration_customized(seg2sens, os_model = 'Rajagopal_2015'):
	...
	''' Perform calibration for the OpenSim model with customized segment-to-sensor transformation (seg2sens) '''
	...
	for segment in seg2sens.keys():
		imu_placement = tree.find('PhysicalOffsetFrame', {'name': constant_mc10.MC10_TO_OPENSENSE_MAP[segment]})
		imu_orientation = imu_placement.find('orientation')

		angle = R.from_matrix(seg2sens[segment]).as_euler(sequence)
