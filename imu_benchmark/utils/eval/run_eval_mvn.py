...
def evaluate(subject, task, reference, mocap_alignment, selected_setup, do_biomodel):
    ''' tbd '''
    ...
    for subject in subject_list:
        ...
        for task in task_list:
            ...
            if selected_setup == 'mm':
                filename_mvn = constant_common.OUT_MT_JA_PATH + 'ik_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + task + '.pkl'
                if do_biomodel:
                    filename_os           = constant_common.OUT_OPENSENSE_JA_PATH + 'ik_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + task + '.pkl'
                    filename_mvn_biomodel = constant_common.OUT_MVN_JA_PATH + 'ik_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + task + '.pkl'
            else:
            ...
            if do_biomodel:
                ...
                for old_key, new_key in rename_keys.items():
                    ja_mvn_opensense[new_key] = ja_mvn_opensense.pop(old_key)

                # XXX: hardcoded as these are not available in OpenSense outputs
                ja_mvn_opensense['knee_adduction_r']  = np.zeros(ja_mvn_opensense['knee_flexion_r'].shape)
                ja_mvn_opensense['knee_adduction_l']  = np.zeros(ja_mvn_opensense['knee_flexion_l'].shape)
                ja_mvn_opensense['knee_rotation_r']   = np.zeros(ja_mvn_opensense['knee_flexion_r'].shape)
                ja_mvn_opensense['knee_rotation_l']   = np.zeros(ja_mvn_opensense['knee_flexion_l'].shape)
                ja_mvn_opensense['ankle_adduction_r'] = np.zeros(ja_mvn_opensense['ankle_flexion_r'].shape)
                ja_mvn_opensense['ankle_adduction_l'] = np.zeros(ja_mvn_opensense['ankle_flexion_l'].shape)
                ja_mvn_opensense['ankle_rotation_r']  = np.zeros(ja_mvn_opensense['ankle_flexion_r'].shape)
                ja_mvn_opensense['ankle_rotation_l']  = np.zeros(ja_mvn_opensense['ankle_flexion_l'].shape)
            ...
            if mocap_alignment:
                ...
                ja_mvn = eval_utils.get_ja_alignment(ja_mvn, ja_mc, alignment_id, task)

                if do_biomodel:
                    ja_mvn_opensense = eval_utils.get_ja_alignment(ja_mvn_opensense, ja_mc, alignment_id, task)
                    ja_mvn_biomodel  = eval_utils.get_ja_alignment(ja_mvn_biomodel, ja_mc, alignment_id, task)
                ...
                # ax[0, 0].plot(ja_mvn['hip_flexion_r'], color = 'r')
                # ax[1, 0].plot(ja_mvn['hip_adduction_r'], color = 'r')
                # ax[2, 0].plot(ja_mvn['hip_rotation_r'], color = 'r')
                # ax[0, 1].plot(ja_mvn['knee_flexion_r'], color = 'r')
                # ax[1, 1].plot(ja_mvn['knee_adduction_r'], color = 'r')
                # ax[2, 1].plot(ja_mvn['knee_rotation_r'], color = 'r')
                # ax[0, 2].plot(ja_mvn['ankle_flexion_r'], color = 'r')
                # ax[1, 2].plot(ja_mvn['ankle_adduction_r'], color = 'r')
                # ax[2, 2].plot(ja_mvn['ankle_rotation_r'], color = 'r')
            ...
            if do_biomodel:
                rmse_mvn_opensense = eval_utils.calculate_rmse(segment_mc, segment_mvn_opensense, do_biomodel = True)
                rmse_mvn_biomodel  = eval_utils.calculate_rmse(segment_mc, segment_mvn_biomodel, do_biomodel = True)
            
            print('- Save the evaluation results')
            common.mkfolder(constant_common.OUT_RMSE_PATH)
            filename_mvn = constant_common.OUT_RMSE_PATH + 'eval_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + task + reference + title_alignment + '_mvn' + '.pkl'
            if do_biomodel:
                filename_mvn_opensense = constant_common.OUT_RMSE_PATH + 'eval_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + task + reference + title_alignment + '_mvn_opensense' + '.pkl'
                filename_mvn_biomodel  = constant_common.OUT_RMSE_PATH + 'eval_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + task + reference + title_alignment + '_mvn_biomodel' + '.pkl'
