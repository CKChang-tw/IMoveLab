# name: run_mvn_biomodel.py
# description: Extract IK results from the MVN biomechanical model
# author: Vu Phan
# date: 2025/01/20
...
def mvn_ik_biomodel(subject, task):
    ...
    for subject in subject_list:
        ...
        for selected_task in task_list:
            ...
            mvn_dt = pd.read_excel(mvn_fn, sheet_name = constant_mvn.MVN_JOINT_ANGLE_SHEET)

            print('- Extract joint angles in the MVN biomechanical model')
            ja_mvn_biomodel = {}
            for joint in constant_mvn.MVN_JOINT_ANGLE_MAP.keys():
                ja_mvn_biomodel[joint] = mvn_dt[constant_mvn.MVN_JOINT_ANGLE_MAP[joint]].to_numpy()
            ...
            if sync_info['first_start'] == 'imu':
                for joint in ja_mvn_biomodel.keys():
                    ja_mvn_biomodel[joint] = ja_mvn_biomodel[joint][sync_info['shifting_id']:]

            print('- Save results of ' + selected_task)
            common.mkfolder(constant_common.OUT_MVN_JA_PATH)
            if selected_setup == 'mm':
                filename = constant_common.OUT_MVN_JA_PATH + 'ik_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + selected_task + title_offset + '.pkl'
            else:
                filename = constant_common.OUT_MVN_JA_PATH + 'ik_s' + str(subject) + '_' + f_type.lower() + '_' + dim.upper() + '_' + selected_task + '_' + selected_setup + title_offset + '.pkl'
            with open(filename, 'wb') as f:
                pickle.dump(ja_mvn_biomodel, f)
